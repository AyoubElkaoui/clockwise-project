...
export default function Sidebar({ currentMonth, timeEntries, className = "" }: SidebarProps): JSX.Element {
    ...
    const navigationItems: NavigationItem[] = [
        { href: "/dashboard", label: "Dashboard", icon: HomeIcon, description: "Overzicht van je week" },
        { href: "/overview", label: "Uren Overzicht", icon: ChartBarIcon, description: "Bekijk je goedgekeurde uren" },
        { href: "/vacation", label: "Vakantie", icon: CalendarDaysIcon, description: "Beheer je vakantieaanvragen" },
        { href: "/profile", label: "Mijn Account", icon: UserCircleIcon, description: "Persoonlijke instellingen" }
    ];
    ...
    const getHoursForDay = (day: number): number => {
        ...
        const dayEntries = timeEntries.filter(entry => entry.startTime && entry.startTime.startsWith(dayStr));
    ...
    return (
        <>
            ...
            <div className="sm:hidden flex justify-between items-center p-4 bg-black border-b border-gray-800">
                ...
                <button className="text-white text-2xl" onClick={() => setShowMobileMenu(!showMobileMenu)}>
                    {showMobileMenu ? "✕" : "☰"}
                </button>
            </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            <div className="w-14 h-14 bg-blue-600/30 rounded-2xl flex items-center justify-center shadow-lg border border-blue-500/40">
                                <span className="text-lg font-bold text-blue-400">{userInitials}</span>
                            </div>
                        ...
                        {navigationItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className={`font-semibold text-sm truncate ${isActive ? "text-white" : "text-blue-400"}`}>
                                            {item.label}
                                        </div>
                                        <div className={`text-xs truncate ${isActive ? "text-blue-200" : "text-gray-500"}`}>
                                            {item.description}
                                        </div>
                                    </div>
                                </Link>
                    ...
                    <div className="mt-6">
                        ...
                        <div className="bg-gray-900 rounded-xl p-4 border border-gray-700">
                            <div className="grid grid-cols-7 gap-1 mb-3 text-xs text-gray-400 font-bold text-center">
                                {["M", "D", "W", "D", "V", "Z", "Z"].map((d, i) => <div key={i}>{d}</div>)}
                            </div>
