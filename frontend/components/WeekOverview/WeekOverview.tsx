...
import { getTimeEntries, updateTimeEntry } from "@/lib/api";
          ...
          {entries.length === 0 ? (
            <div className="text-center py-3">
              <p className="text-xs text-gray-500">
                {isWeekend ? t("week.weekend") : t("week.noHours")}
              </p>
            </div>
          ) : (
            <div className="space-y-2">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs bg-elmar-primary text-white px-2 py-0.5 rounded-full font-medium">
                              {formatHours(hours)} {t("common.hours")}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-1 text-xs text-gray-600">
                          ...
                          <span className="truncate">
                            {entry.project?.name || t("week.unknownProject")}
                          </span>
                        </div>
              ...
              {entries.length > 2 && (
                ...
                >
                  {isExpanded
                    ? `▲ ${t("week.showLess")}`
                    : `▼ ${t("week.showAll", { count: entries.length })}`}
                </button>
              )}
            </div>
          )}
...
export default function WeekOverview() {
  ...
  const handleSaveWeek = async () => {
    ...
    if (conceptEntries.length === 0) {
      showToast(t("week.noConceptEntriesSave"), "error");
      return;
    }
  ...
  const handleSubmitWeek = async () => {
    ...
    if (conceptEntries.length === 0) {
      showToast(t("week.noConceptEntriesSubmit"), "error");
      return;
    }
  ...
  const handlePrevMonth = () =>
    setCurrentWeek((w) =>
      w.subtract(1, "month").startOf("month").startOf("isoWeek"),
    );
  const handleNextMonth = () =>
    setCurrentWeek((w) =>
      w.add(1, "month").startOf("month").startOf("isoWeek"),
    );
  ...
  async function handleEntrySaved() {
    closeModal();
    await fetchFromDB();
    showToast(t("week.hoursSaved"), "success");
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-blue-100 text-lg">
              {viewMode === "week" ? (
                <>
                  {startVanWeek.format("D MMM")} -{" "}
                  {eindVanWeek.format("D MMM YYYY")}
                </>
              ) : (
                <>
                  {monthDays[0]?.format("D MMM")} -{" "}
                  {monthDays[monthDays.length - 1]?.format("D MMM YYYY")}
                </>
              )}
            </p>
            {weekStatus !== "concept" && (
          ...
          <div className="text-right">
            ...
            <div className="text-sm text-blue-200">
              {totalHoursThisWeek < 40
                ? `${formatHours(40 - totalHoursThisWeek)} ${t("week.under40")}`
                : t("week.complete")}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">{t("week.today")}</span>
            </button>
            <button
              className="btn btn-ghost text-white hover:bg-white/20 rounded-xl"
            ...
            >
              ...
              <span className="hidden sm:inline">
                {viewMode === "week" ? t("week.month") : t("week.week")}
              </span>
            </button>
          </div>
              ...
              <>
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {isSaving
                      ? t("register.saving")
                      : t("week.saveCount", { count: conceptEntries.length })}
                  </span>
                </button>
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {isSubmitting
                      ? t("week.submitting")
                      : t("week.submitCount", { count: conceptEntries.length })}
                  </span>
                </button>
              </>
            )}
      ...
      <div
        className={`grid gap-4 ${viewMode === "week" ? "grid-cols-1 lg:grid-cols-7" : "grid-cols-7"}`}
      >
        {(viewMode === "week" ? days : monthDays).map((day, index) => {
          const entries = getDayEntries(day);
          const isCurrentMonth =
            viewMode === "month" && day.month() === currentWeek.month();
          ...
          <div className="card-body p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-blue-100 rounded-xl p-4 text-center">
                ...
                <div className="text-sm text-gray-600">{t("week.average")}</div>
              </div>
              ...
              <div className="bg-blue-100 rounded-xl p-4 text-center">
                ...
                <div className="text-sm text-gray-600">{t("week.of40")}</div>
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-medium text-gray-700">
                  {formatHours(totalHoursThisWeek)} / 40u
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  ...
                  style={{
                    width: `${Math.min((totalHoursThisWeek / 40) * 100, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
