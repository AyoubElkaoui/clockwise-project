...
export default function ManagerApprovePage() {
  ...
  const groupEntriesByStatus = (entries: any[]) => {
    const grouped: { [key: string]: any[] } = {};
    const statusOrder = ["ingeleverd", "goedgekeurd", "afgekeurd"];
    entries.forEach((entry) => {
  ...
  const handleBulkApproveByProject = async (projectId: number) => {
    const projectEntries = filteredEntries.filter(
      (e) => e.project?.id === projectId && e.status === "ingeleverd",
    );
  ...
  // Statistics
  const stats = useMemo(() => {
    const pending = entries.filter((e) => e.status === "ingeleverd").length;
    const approved = entries.filter((e) => e.status === "goedgekeurd").length;
    const rejected = entries.filter((e) => e.status === "afgekeurd").length;
  ...
  const canSelectAll =
    filteredEntries.filter((e) => e.status === "ingeleverd").length > 0;
              ...
              <div className="flex items-center gap-3">
                <Checkbox
                  checked={
                    selectedEntries.size ===
                      filteredEntries.filter((e) => e.status === "ingeleverd")
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                  {selectedEntries.size === 0
                    ? `${filteredEntries.filter((e) => e.status === "ingeleverd").length} items beschikbaar`
                    : `${selectedEntries.size} van ${filteredEntries.filter((e) => e.status === "ingeleverd").length} geselecteerd`}
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                {selectedEntries.size > 0 && (
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {status === "ingeleverd" && statusEntries.length > 0 && (
                        <Button
                          size="sm"
                          onClick={() =>
