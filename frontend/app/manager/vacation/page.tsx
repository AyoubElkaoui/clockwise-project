  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Vakantie Verzoeken</h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">{pendingCount} verzoeken wachten op goedkeuring</p>
        </div>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="flex gap-4">
            ...
            <div className="flex gap-2">
              <Button variant={filterStatus === "pending" ? "default" : "outline"} onClick={() => setFilterStatus("pending")}>In afwachting</Button>
              <Button variant={filterStatus === "approved" ? "default" : "outline"} onClick={() => setFilterStatus("approved")}>Goedgekeurd</Button>
              <Button variant={filterStatus === "rejected" ? "default" : "outline"} onClick={() => setFilterStatus("rejected")}>Afgekeurd</Button>
      ...
      {filteredRequests.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-slate-600 dark:text-slate-400">Geen vakantie verzoeken gevonden</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-600 dark:text-slate-400">{request.user?.function || "Medewerker"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                    <div>
                      ...
                      <p className="font-medium text-slate-900 dark:text-slate-100">{dayjs(request.startDate).format("DD MMM YYYY")}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-600 dark:text-slate-400 mb-1 flex items-center gap-1">
                        <Calendar className="w-3 h-3" />
                      ...
                      <p className="font-medium text-slate-900 dark:text-slate-100">{dayjs(request.endDate).format("DD MMM YYYY")}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-600 dark:text-slate-400 mb-1">Aantal Dagen</p>
                      <p className="font-medium text-purple-600 dark:text-purple-400">{calculateDays(request.startDate, request.endDate)} dagen</p>
                    </div>
                  </div>
                  ...
                  {request.status === "pending" && (
                    selectedRequest?.id === request.id ? (
                      <div className="space-y-3 bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                        ...
                        />
                        <div className="flex gap-2">
                          <Button className="bg-green-600 hover:bg-green-700" onClick={() => handleApprove(request.id, comment)}>
                            <CheckCircle className="w-4 h-4 mr-1" />
                            Goedkeuren
                          </Button>
                          <Button variant="outline" className="text-red-600 border-red-600" onClick={() => handleReject(request.id, comment)}>
                            <XCircle className="w-4 h-4 mr-1" />
                            Afkeuren
                          </Button>
                          <Button variant="ghost" onClick={() => { setSelectedRequest(null); setComment(""); }}>
                            Annuleren
                          </Button>
                        </div>
                      </div>
